...
testcases="${testcases} fixture-1850.yaml"
for opts in  ""  ; do
for testcase in ${testcases} ; do
    ...
    if [[ -z ${opts} ]]; then
        ...
        target=./gen-${testcase%.*}-full
    fi
    serverName="codegensrv"
    rm -rf ${target} ${target}"-client"
    mkdir ${target} ${target}"-client"
    ...
    swagger generate client --skip-validation ${opts} --spec ${spec} --target ${target}"-client"  1>${testcase%.*}.log 2>&1
    # 1>x.log 2>&1
    #
    if [[ $? != 0 ]] ; then
