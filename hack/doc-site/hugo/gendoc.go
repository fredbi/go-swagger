...
func main() {
	...
	fmt.Println("==> Generated goswagger.yaml")
	...
	fmt.Println("    Visit: http://localhost:1314/goswagger/")
	fmt.Println()
	...
	cmd := exec.CommandContext(ctx, "hugo", "server",
		...
		"--bind", "0.0.0.0",
		"--port", "1313",
		"--baseURL", "http://localhost:1313/go-swagger/",
		"--appendPort=false",
		"--logLevel", "info",
		"--cleanDestinationDir",
	)
...
// generateRuntimeYAML reads the template and writes goswagger.yaml with substitutions.
func generateRuntimeYAML(goVersion, latestRelease, versionMessage, buildTime string) {
	tmpl, err := os.ReadFile("goswagger.yaml.template")
	if err != nil {
	...
	if err := os.WriteFile("goswagger.yaml", []byte(out), 0o600); err != nil { //nolint:mnd
		fatalf("writing goswagger.yaml: %v", err)
	}
}
