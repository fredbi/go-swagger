...
It ships the runtime that supports what it generates.
Our focus is to produce idiomatic, fast go code that plays nice with golint, go vet etc.
...
`go-swagger` is currently feature complete and has stabilized its API. Most features and building blocks are
...
The toolkit is highly customizable and allows endless possibilities to work with OpenAPI 2.0 specifications.
...
Here a few illustrated basic usages. More details are available [there](./use-cases)
...
Just like swagger, this does not cover code generated by the toolkit.
