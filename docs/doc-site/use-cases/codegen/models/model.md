...
weight: 40
description: Produce go types from a swagger spec
---

## Generate models from a swagger spec

This command generates only go types ("models") from the types that your API spec defines ("#/definitions").
...
[model command options]
      -n, --name=                                                                            the model to generate, repeat for multiple (defaults to all). Same as --models
          --accept-definitions-only                                                          accepts a partial swagger spec with only the definitions key

    Options common to all code generation commands:
<<<<<<< HEAD:docs/generate/model.md
...
>>>>>>> 945d7476e (new doc site):docs/doc-site/generate/model.md
