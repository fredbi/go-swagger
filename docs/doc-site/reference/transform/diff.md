...
The toolkit has a command that will let you find the changes in a spec, and whether they may break existing clients.
...
1. Humans imagine an API.
2. Humans code the server and one or more clients.
3. Humans write tests to ensure that all the humans are doing the right thing.

This is okay, but has challenges of its own.

1. There's a lot of repetitive boilerplate that humans are forced to write.
2. The tests can provide feedback which is late or inaccurate. Depending on the actual test
   they may or may not detect backwards incompatible changes. When they do detect it they could
   flag a failure at a point which is removed from the bug injection. ie I break the server API
   but only find out later when one of the client API tests fails (maybe).
...
Methodologies like [Consumer-Driven Contract Testing](https://docs.pact.io/how_pact_works) (PACT) can give confidence that the API has not been broken... but it suffers from some challenges:
...
2. Depending on the sequence of execution a client change could cause the server CI/CD pipeline to asynchronously fail as a result of a breaking client change.
...
why not identify the problem at its source: the spec itself. If the only thing that can influence the interaction is the API, and the spec defines the API - let's focus on that.
...
1. The server expects something new from the client which it doesn't provide.
2. The server stops accepting data formats that the client provides.
3. The server stops accepting requests on a given endpoint that the client expects to call.
4. The server changes the format of returned data.
5. The server returns a different return code or different data.
...
If we ensure that each version of the spec is backwards compatible with its predecessor then we can use semantic versioning to ensure that clients on a given version of a spec in production will never be broken by an API change in production.
...
These cases need to be handled VERY CAREFULLY. Let's take adding the enum value.

1. You add a new option to `cooked_egg` by adding "FRIED" to the existing enum `["POACHED","SCRAMBLED"]`.
2. You run `swagger diff` on your new spec and it says:
...
3. You now have some choices:
   - revert, revert, revert - back off and not break the spec
   - create a new version of the spec in the server and start to migrate clients to that
   - do a managed migration
4. Let's assume you've decided VEWWWY VEWWWY CAREFUWWY migrate your change. Here's how that would work...
...
1. You add a new option to `cooked_egg` by adding "FRIED" to the existing enum `["POACHED","SCRAMBLED"]`.
2. You run `swagger diff` on your new spec and it complains as above.
3. You now generate a diff list using `swagger diff -f json` to produce a json formatted diff list.
4. You copy the breaking change you wish to ignore into a `swagger_diff_ignore_file.json`.
5. You ensure your build is configured to read this ignore file so now your build will no longer break.
6. VERY IMPORTANT: Now you must specifically put a temporary conditional check plus tests in the server to ensure you don't return this new enum value... YET. This gives you a chance to migrate clients to the version with the new enum value.
7. Deploy the change and then update the clients to use the new version (unless your clever build pipeline does that automatically because you use semantic versioning on your spec).
8. Once all clients have been updated (**and deployed**) you can remove the item from your ignore file and the code that blocks the server returning the new value.
...
Will that break the client? It... depends. If the client has a firm set of responses it expects then perhaps, yes. New error responses are less likely to break the client as there are usually "Something unknown has gone sour" error handling to cope with that.
