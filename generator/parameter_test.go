...
func TestGenParameter_Issue303(t *testing.T) {
	services := map[string][]string{
		"giveFruit": {
			`	if err := validate.EnumCase("fruit", "query", o.Fruit, []any{"Apple", "Pear", "Plum"}, false); err != nil {`,
		},
		"giveFruitBasket": {
			`		if err := validate.EnumCase(fmt.Sprintf("%s.%v", "fruit", i), "query", fruitIIC, []any{[]any{"Strawberry", "Raspberry"}, []any{"Blueberry", "Cranberry"}}, false); err != nil {`,
			`				if err := validate.EnumCase(fmt.Sprintf("%s.%v", fmt.Sprintf("%s.%v", "fruit", i), ii), "query", fruitII, []any{"Peach", "Apricot"}, false); err != nil {`,
			`	if err := validate.EnumCase("fruit", "query", o.Fruit, []any{[]any{[]any{"Banana", "Pineapple"}}, []any{[]any{"Orange", "Grapefruit"}, []any{"Lemon", "Lime"}}}, false); err != nil {`,
		},
	}
	...
	fixtureConfig := map[string]map[string][]string{
		...
		"3": {
			"serverParameter": {
				...
				`if err := validate.EnumCase(fmt.Sprintf("%s.%v", fmt.Sprintf("%s.%v", fmt.Sprintf("%s.%v", "isAnOption4", i), ii), iii), "query", isAnOption4III.String(), []any{"a", "b", "c"}, true); err != nil {`,
	...
	fixtureConfig := map[string]map[string][]string{
		...
		"3": { // fixture index
			"serverParameter": { // executed template
				...
				`	if err := validate.EnumCase("myObject", "body", o.MyObject.String(), []any{"1992-01-01", "2012-01-01"}, true); err != nil {`,
		...
		"4": { // fixture index
			"serverParameter": { // executed template
				...
				`	if err := validate.EnumCase("myObject", "body", o.MyObject, []any{[]any{[]any{"1992-01-01", "2012-01-01"}}},`,
				`		true); err != nil {`,
				`		return err`,
				`	myObjectIC := o.MyObject`,
				...
				`				if err := validate.EnumCase(fmt.Sprintf("%s.%v", fmt.Sprintf("%s.%v", "myObject", i), ii), "", myObjectII.String(), []any{"1992-01-01", "2012-01-01"}, true); err != nil {`,
				`					return err`,
				...
				`		if err := validate.EnumCase(fmt.Sprintf("%s.%v", "myObject", i), "body", myObjectIIC, []any{[]any{"1992-01-01", "2012-01-01"}},`,
				`			true); err != nil {`,
				`	o.MyObject = myObjectIR`,
			},
		},
	...
	fixtureConfig := map[string]map[string][]string{
		...
		"getNestedWithValidations": { // fixture index
			"serverParameter": { // executed template
				...
				"	HTTPRequest *http.Request `json:\"-\"`",
				`	GenericNestedWithValidations [][][][]any`,
				...
				`	var genericNestedWithValidationsIR [][][][]any`,
				...
				`			var genericNestedWithValidationsIIR [][][]any`,
				...
				`					var genericNestedWithValidationsIIIR [][]any`,
				...
				`							var genericNestedWithValidationsIIIIR []any`,
	...
	fixtureConfig := map[string]map[string][]string{
		...
		"getSimpleArrayWithSliceValidation": { // fixture index
			"serverParameter": { // executed template
				...
				`	if err := validate.EnumCase("simpleArrayWithSliceValidation", "body", o.SimpleArrayWithSliceValidation, []any{[]any{1, 2, 3}, []any{4, 5, 6}},`,
				`		true); err != nil {`,
			},
		},
