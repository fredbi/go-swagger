...
func TestGenerateModel_Issue743(t *testing.T) {
	...
	assertInCode(t, "jsonutils.WriteJSON(m.Foo)", res)
	assertInCode(t, "jsonutils.WriteJSON(m.Bar)", res)
	assertInCode(t, "jsonutils.ReadJSON(raw, &aO0)", res)
	assertInCode(t, "jsonutils.ReadJSON(raw, &aO1)", res)
