...
func TestGenClientFixtures(t *testing.T) {
	t.Parallel()
	defer discardOutput()()
	...
	t.Run("generate client", func(t *testing.T) {
		for idx, tt := range clientFixtures() {
			i := idx
			t.Run(tt.name, func(t *testing.T) {
				...
				opts.Target = filepath.Join(
					base,
					opts.LanguageOpts.ManglePackageName(tt.name, "client_test"+strconv.Itoa(i)),
				)
				err := os.MkdirAll(opts.Target, readableDir)
				require.NoError(t, err)
...
func TestGenClient_2471(t *testing.T) {
	...
	fixtureConfig := map[string][]string{
		"client/operations/example_post_parameters.go": { // generated file
			...
			` 	  fooIS := stringutils.JoinByFormat(fooIC, "")`,
			` 	  return fooIS`,
			...
			` 	    fooPathIS := stringutils.JoinByFormat(fooPathIC, "")`,
			`  return fooPathIS`,
			...
			` 	    fooQueryIS := stringutils.JoinByFormat(fooQueryIC, "")`,
			`  return fooQueryIS`,
		},
	}
...
func TestGenClient_909_3(t *testing.T) {
	...
	fixtureConfig := map[string][]string{
		"client/operations/get_optional_parameters.go": { // generated file
			...
			`		isAnOption2IIS := stringutils.JoinByFormat(isAnOption2IIC, "")`,
			`		isAnOption2IIV := isAnOption2IIS[0]`,
			`		isAnOption2IC = append(isAnOption2IC, isAnOption2IIV)`,
			`	isAnOption2IS := stringutils.JoinByFormat(isAnOption2IC, "pipes")`,
			...
			`			isAnOption4IIIS := stringutils.JoinByFormat(isAnOption4IIIC, "pipes")`,
			`			isAnOption4IIIV := isAnOption4IIIS[0]`,
			`			isAnOption4IIC = append(isAnOption4IIC, isAnOption4IIIV)`,
			`		}`,
			`		isAnOption4IIS := stringutils.JoinByFormat(isAnOption4IIC, "tsv")`,
			`		isAnOption4IIV := isAnOption4IIS[0]`,
			`		isAnOption4IC = append(isAnOption4IC, isAnOption4IIV)`,
			`	isAnOption4IS := stringutils.JoinByFormat(isAnOption4IC, "csv")`,
			...
			`		isAnOptionalHeaderIIS := stringutils.JoinByFormat(isAnOptionalHeaderIIC, "")`,
			`		isAnOptionalHeaderIIV := isAnOptionalHeaderIIS[0]`,
			`		isAnOptionalHeaderIC = append(isAnOptionalHeaderIC, isAnOptionalHeaderIIV)`,
			`	isAnOptionalHeaderIS := stringutils.JoinByFormat(isAnOptionalHeaderIC, "pipes")`,
			...
			`		notAnOption1IIS := stringutils.JoinByFormat(notAnOption1IIC, "pipes")`,
			`		notAnOption1IIV := notAnOption1IIS[0]`,
			`		notAnOption1IC = append(notAnOption1IC, notAnOption1IIV)`,
			`	notAnOption1IS := stringutils.JoinByFormat(notAnOption1IC, "csv")`,
...
func TestGenClient_909_5(t *testing.T) {
	...
	fixtureConfig := map[string][]string{
		"client/operations/get_optional_responses.go": { // generated file
			...
			`		valxIsAnOptionalHeader0DirtSimpleInteger, err := conv.ConvertInt64(hdrXIsAnOptionalHeader0DirtSimpleInteger)`,
			...
			`		valxIsAnOptionalHeader0Primitive, err := conv.ConvertFloat32(hdrXIsAnOptionalHeader0Primitive)`,
			...
			`	xIsAnOptionalHeader0DirtSimpleArrayIR := stringutils.SplitByFormat(xIsAnOptionalHeader0DirtSimpleArrayIV, "")`,
			`	for i, xIsAnOptionalHeader0DirtSimpleArrayIIV := range xIsAnOptionalHeader0DirtSimpleArrayIR {`,
			`		val, err := conv.ConvertInt64(xIsAnOptionalHeader0DirtSimpleArrayIIV)`,
			...
			`	xIsAnOptionalHeader1IR := stringutils.SplitByFormat(xIsAnOptionalHeader1IV, "tsv")`,
			...
			`	xIsAnOptionalHeader2IR := stringutils.SplitByFormat(xIsAnOptionalHeader2IV, "")`,
			`	for _, xIsAnOptionalHeader2IIV := range xIsAnOptionalHeader2IR {`,
			`			xIsAnOptionalHeader2IIC []int32`,
			`		xIsAnOptionalHeader2IIR := stringutils.SplitByFormat(xIsAnOptionalHeader2IIV, "pipes")`,
			`		for ii, xIsAnOptionalHeader2IIIV := range xIsAnOptionalHeader2IIR {`,
			`			val, err := conv.ConvertInt32(xIsAnOptionalHeader2IIIV)`,
			...
			`	xIsAnOptionalHeader2NoFormatIR := stringutils.SplitByFormat(xIsAnOptionalHeader2NoFormatIV, "pipes")`,
			`	for _, xIsAnOptionalHeader2NoFormatIIV := range xIsAnOptionalHeader2NoFormatIR {`,
			`			xIsAnOptionalHeader2NoFormatIIC []int64`,
			`		xIsAnOptionalHeader2NoFormatIIR := stringutils.SplitByFormat(xIsAnOptionalHeader2NoFormatIIV, "tsv")`,
			`		for ii, xIsAnOptionalHeader2NoFormatIIIV := range xIsAnOptionalHeader2NoFormatIIR {`,
			`			val, err := conv.ConvertInt64(xIsAnOptionalHeader2NoFormatIIIV)`,
			...
			`	xIsAnOptionalHeader3IR := stringutils.SplitByFormat(xIsAnOptionalHeader3IV, "")`,
			`	for _, xIsAnOptionalHeader3IIV := range xIsAnOptionalHeader3IR {`,
			`			xIsAnOptionalHeader3IIC [][]strfmt.UUID`,
			`		xIsAnOptionalHeader3IIR := stringutils.SplitByFormat(xIsAnOptionalHeader3IIV, "pipes")`,
			`		for _, xIsAnOptionalHeader3IIIV := range xIsAnOptionalHeader3IIR {`,
			`				xIsAnOptionalHeader3IIIC []strfmt.UUID`,
			`			xIsAnOptionalHeader3IIIR := stringutils.SplitByFormat(xIsAnOptionalHeader3IIIV, "")`,
...
func TestGenClient_909_6(t *testing.T) {
	...
	fixtureConfig := map[string][]string{
		"client/operations/get_optional_responses.go": { // generated file
			...
			`	if hdrXaBool != "" {`,
			`		valxABool, err := conv.ConvertBool(hdrXaBool)`,
			`		if err != nil {`,
			...
			`	if hdrXaDouble != "" {`,
			`		valxADouble, err := conv.ConvertFloat64(hdrXaDouble)`,
			`		if err != nil {`,
			...
			`	if hdrXaFloat != "" {`,
			`		valxAFloat, err := conv.ConvertFloat32(hdrXaFloat)`,
			`		if err != nil {`,
			...
			`	if hdrXaUint32 != "" {`,
			`		valxAUint32, err := conv.ConvertUint32(hdrXaUint32)`,
			`		if err != nil {`,
			...
			`	if hdrXaUint64 != "" {`,
			`		valxAUint64, err := conv.ConvertUint64(hdrXaUint64)`,
			`		if err != nil {`,
...
func prepareClientTarget(t *testing.T, root string) string {
	target := clientDir(root)
	require.NoError(t, os.MkdirAll(target, readableDir))
	t.Run("go mod init", gentest.GoModInit(target))

	return target
}

func clientDir(root string) string {
	return filepath.Join(root, randWithPattern("generated"))
}

func assertImports(t *testing.T, baseImport, code string) {
